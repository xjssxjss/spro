...
public class ExcelReadKit {
    ...
    public static ExcelReadResultBean realSheetValue(Sheet sheet) {
        ExcelReadResultBean resultBean = new ExcelReadResultBean();
        boolean result = true;
        String errMsg = "";
                            ...
                            if (cell != null) {// 判断cell是否为空
                                if (cell.getCellType() == ExcelConfig.CT_NUMERIC) {
                                    if (HSSFDateUtil.isCellDateFormatted(cell)) {// 判断是否日期类型
                                        ...
                                        //DateKit.formatDate(dateValue, DateKit.DEFAULT_DATE_TIME_FORMAT)\
                                        rowList.add(DateUtil.parseDateToStr(new Date(),DateUtil.DATE_TIME_FORMAT_YYYY_MM_DD_HH_MI_SS));
                                    } else {
